...
const mpd = {
  ...
  setup() {
    ...
    const player = this; // Config hacks
    ...
    player.config.i18n.qualityLabel[mpd.qualityAutoMagicValue] = i18n.get('qualityAuto', player.config); // Set speed options from config
    ...
    Object.defineProperty(player.media, 'quality', {
      ...
      set(input) {
        ...
        if (input === mpd.qualityAutoMagicValue) {
          ...
          const currentIndex = player.dash.getQualityFor('video');
          const currentHeight = currentIndex ? player.dash.getBitrateInfoListFor('video')[currentIndex].height : 0; // Sorting bitrates by DESC

          const bitrateList = player.dash.getBitrateInfoListFor('video');
          bitrateList.sort((a, b) => b.bitrate - a.bitrate); // Find quality

          const quality = bitrateList.find(e => e.height === input); // Disabling auto switch quality

          dashConfig.streaming.abr.autoSwitchBitrate.video = false;
          player.dash.updateSettings(dashConfig); // Update quality
    ...
    Object.defineProperty(player.media, 'audioTrack', {
      ...
      set(input) {
        ...
        if (match) {
          ...
          const track = player.dash.getTracksFor('audio').find(e => e.index === index); // Update video track
          ...
          // Find by id
          const track = player.dash.getTracksFor('audio').find(e => e.id === input); // Update audio track
    ...
    Object.defineProperty(player.media, 'videoTrack', {
      ...
      set(input) {
        ...
        if (match) {
          // Find by index
          const index = parseInt(match[1], 10);
          const track = player.dash.getTracksFor('video').find(e => e.index === index); // Update video track
          ...
          // Find by id
          const track = player.dash.getTracksFor('video').find(e => e.id === input); // Update video track
...
const controls = {
  ...
  updateSeekTooltip(event) {
    ...
    const point = this.config.markers?.points?.find(({
      time: t
  ...
  setAudioTrackMenu(options) {
    ...
    const list = this.elements.settings.panels.audioTrack.querySelector('[role="menu"]'); // Set options if passed and filter based on uniqueness and config
    ...
    this.options.audioTrack.sort((a, b) => {
  ...
  setVideoTrackMenu(options) {
    ...
    const list = this.elements.settings.panels.videoTrack.querySelector('[role="menu"]'); // Set options if passed and filter based on uniqueness and config
    ...
    this.options.videoTrack.sort((a, b) => {
  ...
  create(data) {
    ...
    if (this.isMPD) {
      ...
      this.config.i18n.videoTrackMPDLabel = mpd.getVideoTrackLabels.call(this); // Set available tracks and quality levels
  ...
  setMarkers() {
    ...
    const points = this.config.markers?.points?.filter(({
      time
...
const captions = {
  ...
  getTracks(update = false) {
    ...
    return tracks.filter(track => !this.isHTML5 || !this.isMPD || update || this.captions.meta.has(track)).filter(track => ['captions', 'subtitles'].includes(track.kind));
  },
...
const defaults = {
  ...
  'statechange', // Quality
  'qualitychange', // MPEG-DASH
  'audiotrackchange', 'videotrackchange', 'qualitylistupdate', 'audiotracklistupdate', 'videotracklistupdate', 'audiotracklabelsupdate', 'videotracklabelsupdate', // Ads
...
const ui = {
  ...
  build() {
    ...
    toggleClass(this.elements.container, this.config.classNames.pip.supported, support.pip && (this.isHTML5 || this.isMPD) && this.isVideo); // Check for airplay support
...
class Ads {
  ...
  get enabled() {
    ...
    return (this.player.isHTML5 || this.player.isMPD) && this.player.isVideo && config.enabled && (!is.empty(config.publisherId) || is.url(config.tagUrl));
  }
  /**
   * Load the IMA SDK
   */
...
class Plyr {
  constructor(target, options) {
    ...
    switch (_type) {
      ...
      case 'audio':
        this.type = _type;
        this.provider = this.dash ? providers.mpd : providers.html5; // Get config from attributes
  ...
  set audioTrack(input) {
    ...
    let audioTrack = [!is.empty(input) && input, this.storage.get('audioTrack'), config.selected, config.default].find(e => e);
    ...
    if (!options.includes(audioTrack)) {
      ...
      audioTrack = value; // Don't update storage if quality is not supported

      updateStorage = false;
    } // Update config


    config.selected = audioTrack; // Set audio track

    this.media.audioTrack = audioTrack; // Save to storage
  ...
  set videoTrack(input) {
    ...
    let videoTrack = [!is.empty(input) && input, this.storage.get('videoTrack'), config.selected, config.default].find(e => e);
    let updateStorage = true;
    ...
    if (!options.includes(videoTrack)) {
      ...
      videoTrack = value; // Don't update storage if quality is not supported

      updateStorage = false;
    } // Update config


    config.selected = videoTrack; // Set video track

    this.media.videoTrack = videoTrack; // Save to storage
